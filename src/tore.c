...
bool create_schema(sqlite3 *db, const char *tore_path)
{
    ...
    for (; index < ARRAY_LEN(migrations); ++index)
    {
        ...
        if (tore_trace_migration_queries)
            printf("%s\n", migrations[index]);
...
bool show_expanded_notifications_by_index(sqlite3 *db, size_t index)
{
    ...
    if (!load_active_notifications_of_group(db, gns.items[index].group_id, &ns))
        return_defer(false);
...
bool dismiss_grouped_notifications_by_indices_from_args(sqlite3 *db, int *how_many_dismissed, int argc, char **argv)
{
    ...
    while (argc > 0)
    {
        ...
        if (!dismiss_grouped_notification_by_group_id(db, gns.items[index].group_id))
            return_defer(false);
        if (how_many_dismissed)
            *how_many_dismissed += gns.items[index].group_count;
    }
...
const char *render_period_as_sqlite3_datetime_modifier_temp(Period period, unsigned long period_length)
{
    ...
    case PERIOD_NONE:
        return NULL;
    case PERIOD_DAY:
        return temp_sprintf("+%lu days", period_length);
    case PERIOD_WEEK:
        return temp_sprintf("+%lu days", period_length * 7);
    case PERIOD_MONTH:
        return temp_sprintf("+%lu months", period_length);
    case PERIOD_YEAR:
        return temp_sprintf("+%lu years", period_length);
    case COUNT_PERIODS:
    default:
        UNREACHABLE("render_period_as_sqlite3_datetime_modifier_temp");
    }
}
...
bool show_active_reminders(sqlite3 *db)
{
    ...
    if (!load_active_reminders(db, &reminders))
        return_defer(false);
...
bool remove_reminder_by_number(sqlite3 *db, int number)
{
    ...
    if (!remove_reminder_by_id(db, reminders.items[number].id))
        return_defer(false);
...
bool verify_date_format(const char *date)
{
    ...
    for (; *format && *date; format++, date++)
    {
        ...
        case '-':
            ...
            break;
        default:
            UNREACHABLE("verify_date_format");
        }
    }
    return !(*format || *date);
}
...
bool checkout_run(Command *self, const char *program_name, int argc, char **argv)
{
    ...
    if (!txn_begin(db))
        return_defer(false);
    if (!fire_off_reminders(db))
        return_defer(false);
    if (!show_active_notifications(db))
        return_defer(false);
...
bool dismiss_run(Command *self, const char *program_name, int argc, char **argv)
{
    ...
    if (!dismiss_grouped_notifications_by_indices_from_args(db, &how_many_dismissed, argc, argv))
        return_defer(false);
    if (!show_active_notifications(db))
        return_defer(false);
...
bool notify_run(Command *self, const char *program_name, int argc, char **argv)
{
    ...
    if (!create_notification_with_title(db, title))
        return_defer(false);
    if (!show_active_notifications(db))
        return_defer(false);
...
bool forget_run(Command *self, const char *program_name, int argc, char **argv)
{
    ...
    if (!remove_reminder_by_number(db, number))
        return_defer(false);
    if (!show_active_reminders(db))
        return_defer(false);
defer:
    if (db)
    {
...
bool remind_run(Command *self, const char *program_name, int argc, char **argv)
{
    ...
    if (!txn_begin(db))
        return_defer(false);
    if (!create_new_reminder(db, title, scheduled_at, period, period_length))
        return_defer(false);
    if (!show_active_reminders(db))
        return_defer(false);
...
bool expand_run(Command *self, const char *program_name, int argc, char **argv)
{
    ...
    int index = atoi(shift(argv, argc));
    if (!show_expanded_notifications_by_index(db, index))
        return_defer(false);
defer:
    if (db)
    {
...
int main(int argc, char **argv)
{
    ...
    for (size_t i = 0; i < ARRAY_LEN(commands); ++i)
    {
        if (strcmp(commands[i].name, command_name) == 0)
        {
            if (!commands[i].run(&commands[i], program_name, argc, argv))
                return_defer(1);
            return_defer(0);
        }
    }
