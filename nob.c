...
int main(int argc, char **argv)
{
    ...
    if (strcmp(command_name, "run") == 0 || strcmp(command_name, "chroot") == 0)
    {
        ...
        if (current_dir == NULL)
            return 1;
        if (!set_environment_variable("HOME", temp_sprintf("%s/" BUILD_FOLDER, current_dir)))
            return 1;
        if (!set_environment_variable("TORE_TRACE_MIGRATION_QUERIES", "1"))
            return 1;
